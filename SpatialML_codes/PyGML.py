...
class PyGBoostBuilder:
    """
    Python implementation of geographic boosting model (PyGBoost).

    Parameters
    ----------
    model_type: {"xgboost", "lightgbm"}, default="xgboost"
        The type of boosting model to use.
        - If "xgboost", uses XGBRegressor from XGBoost.
        - If "lightgbm", uses LGBMRegressor from LightGBM.
    band_width: int or float
        - The number (int) of neighbors for fitting local models if kernel is "adaptive".
        - The number (int or float) of distance within which the neighbors are used for model fitting if kernel is "fixed".
    n_estimators: int, default=100
    ...
    learning_rate: int or float, default=0.1
        ...
        More details please refer to the documentation of xgboost at the link: https://xgboost.readthedocs.io/en/stable/python/python_api.html and https://lightgbm.readthedocs.io/en/latest/pythonapi/lightgbm.LGBMRegressor.html#lightgbm.LGBMRegressor.
    kernel: {"adaptive", "fixed"}, default="adaptive"
        The type of kernel used for determining the neighbors of a data point. Two types are available:
        - If "adaptive", a specific number of neighbors to use for fitting local models.
        - If "fixed", neighbors within a fixed distance for fitting local models.
    ...
    random_state: int, instance of Numpy RandomState or None, default=None
        ...
        More details please refer to the documentation of xgboost at the link: https://xgboost.readthedocs.io/en/stable/python/python_api.html and https://lightgbm.readthedocs.io/en/latest/pythonapi/lightgbm.LGBMRegressor.html#lightgbm.LGBMRegressor.
    **kwargs : dict, optional
        Additional parameters for fitting the xgboost model. Some common parameters include:        
        - max_depth : int, default=None
        ...
        Additional parameters can be found in the xgboost at the link: https://xgboost.readthedocs.io/en/stable/python/python_api.html and https://lightgbm.readthedocs.io/en/latest/pythonapi/lightgbm.LGBMRegressor.html#lightgbm.LGBMRegressor.
    """

    def __init__(self, model_type="xgboost", band_width =60, n_estimators=100, learning_rate=0.1, kernel="adaptive", train_weighted=True, predict_weighted=True,
                 resampled=True, n_jobs=None, random_state=None, **kwargs):
    ...
    def _create_model(self):
        """Create a model based on the selected model type."""
        ...
        for i in range(len(X_train)):
            ...
            if self.train_weighted:
                if self.resampled and len(local_X_train) < 2 * self.n_estimators:
                    ...
                    local_y_train_resampled = pd.concat([local_y_train, more_y_train_resampled], ignore_index=True)
                    sample_weights_resampled = np.concatenate((sample_weights, more_sample_weights_resampled))
                    local_model.fit(local_X_train_resampled, local_y_train_resampled, sample_weight=sample_weights_resampled)
                else:
                    local_model.fit(local_X_train, local_y_train, sample_weight=sample_weights)
            else:
                if self.resampled and len(local_X_train) < 2 * self.n_estimators:
                    resampled_length = min(2 * self.n_estimators, 2*len(local_X_train)) - len(local_X_train)
                    more_X_train_resampled, more_y_train_resampled = resample(local_X_train,
                    ...
                    local_X_train_resampled = pd.concat([local_X_train, more_X_train_resampled], ignore_index=True)
                    local_y_train_resampled = pd.concat([local_y_train, more_y_train_resampled], ignore_index=True)
                    local_model.fit(local_X_train_resampled, local_y_train_resampled)
                else:
    ...
    def get_local_feature_importance(self , importance_type='gain'):
        """
        Get the local feature importance based on local models using specified importance type.

        Parameters:
        -------
        importance_type: str, default='gain'
        The type of feature importance to extract. Options are:
        - 'gain': The average gain of the feature when it is used in trees.
        - 'split': The number of times the feature is used in trees (default for feature_importances_).
        ...
        for i in range(len(self.local_models)):
            ...
            if self.model_type == "xgboost":
                importance_dict = local_model.get_booster().get_score(importance_type=importance_type)
                importance_list = [importance_dict.get(f, 0) for f in self.train_data_columns]

            elif self.model_type == "lightgbm":
                importance_list = local_model.booster_.feature_importance(importance_type=importance_type)

            this_row.extend(importance_list)
            feature_importance_df = pd.concat([feature_importance_df, pd.DataFrame([this_row], columns=column_list)], ignore_index=True)
